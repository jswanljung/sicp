...
(define (install-real-package)
  ...
  (define (raise-to-real x)
    (cond ((number? x) x)
          ((is-top? x) (error "Cannot make real from value: " x))
          (else (raise-to-real (raise x)))))
  (define (make x)
    (tag (exact->inexact (raise-to-real x))))
...
(define (install-rectangular-package)
  ...
  (define (make-from-real-imag x y)
    (cons x y))
  (define (magnitude z)
    (generic-sqrt (add (square (real-part z)) (square (imag-part z)))))
  (define (angle z)
...
(define (install-polar-package)
  ...
  (define (make-from-real-imag x y)
    (cons (generic-sqrt (add (square x) (square y))) (generic-atan y x)))
  ;; interface to the rest of the system
  (define (tag x)
    (attach-tag 'polar x))
...
(define (install-complex-package)
  ...
  (put 'project 'complex (lambda (z) (real-part z))) ; TODO update make-real to deal with other types.
  'done)
  ...
  (let ([type-tags (map type-tag args)])
    ...
    (let ([proc (get op type-tags)])
      (cond ((not (null? proc))
            ...
            ((not (alleq? type-tags))
             (apply apply-generic op (raise-args-to-common-level args)))
            ((not (is-top? (car args)))
             (apply apply-generic op (map raise args)))
            (else (no-match-error))))))
...
(define (make-integer n)
  ((get 'make 'integer) n))
(define (make-real n) ; TODO update make-real to accept other numbers?
  ((get 'make 'real) n))
...
(define (square x)
  (mul x x)) ; TODO redefine to accept general numbers
...
;(add 3 z)
(define z2 (make-complex-from-real-imag 2.3 -4))
(add z z2)
(define z3 (make-complex-from-real-imag  (make-rational 1 2) (make-integer -2)))
;; Still plenty of weirdness here. Rationals don't fit neatly between integers and rationals in practice,
;; only in theory. Now you can have integers and rationals as parts of a complex number, but
;; if you try to add a rational to a complex with a rational real part the result won't have a rational real
;; part because it goes via real when it gets raised. But the whole tower of number types is very shaky anyway.
;; Glad to put this set of exercises behind me.
